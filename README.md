...
Hydrophone-oriented evaluation pipeline built on top of U-FFIA. There are now two practical recommended paths:

- Hydrophone-only: `MobileNetV2 binary none/yem fine-tune + hydrophone preprocess + hydrophone_v1 adaptation`
- Local paired audio+video data: `fast late fusion` over existing audio/video prediction CSVs
...
| Weak-positive note | `yem2` is weaker than `yem1`; fish ate less and feeding happened later |
...
| Fast AV fusion | Accuracy `0.768`, macro-F1 `0.761`, `yem` F1 `0.721` |
| Recommended profile | `binary MobileNetV2 + hydrophone preprocess + hydrophone_v1 adaptation` |
| Recommended local AV path | `python tools/train_fast_late_fusion.py --source-weight voice_yem2=0.8` |
...
- Fast local audio+video fusion details: [docs/FAST_FUSION.md](docs/FAST_FUSION.md)
- Google Colab execution guide: [docs/COLAB.md](docs/COLAB.md)
